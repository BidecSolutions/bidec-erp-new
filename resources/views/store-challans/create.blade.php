@php
    use App\Helpers\CommonHelper;
@endphp

@extends('layouts.layouts')

@section('content')
    <div class="well_N">
        <div class="boking-wrp dp_sdw">
            <div class="row">
                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                    {{ CommonHelper::displayPageTitle('Add New Store Challan') }}
                </div>
                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 text-right">
                    <a href="{{ route('store-challans.index') }}" class="btn btn-success btn-xs">+ View List</a>
                </div>
            </div>
            <div class="row">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <label>Department Detail</label>
                    <select name="department_id" id="department_id" class="form-control select2"
                        onchange="loadMaterialRequestDetailUsingDepartmentId()">
                        <option value="">Select Department</option>
                        @foreach ($getDepartmentList as $gdlRow)
                            <option value="{{ $gdlRow->id }}">{{ $gdlRow->department_name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            <div class="lineHeight">&nbsp;</div>
            <div class="loadStoreChallanDetailSection mt-3"></div>
        </div>
    </div>
@endsection

@section('script')
    <script>
        function loadMaterialRequestDetailUsingDepartmentId() {
            $('.po-selection').select2();

            var departmentId = $('#department_id').val();
            if (departmentId === '') {
                alert('Please Select Department Detail');
                $('.loadStoreChallanDetailSection').html('');
            } else {
                $('.loadStoreChallanDetailSection').html(
                    '<div class="row"><div class="col-lg-12 text-center"><div class="loader"></div>'
                );

                $.ajax({
                    url: '{{ url('/store-challans/getMaterialRequestsByDepartmentId') }}',
                    type: "GET",
                    data: {
                        departmentId: departmentId
                    },
                    success: function(data) {
                        if (data.success) {
                            $('.loadStoreChallanDetailSection').html(data.html);
                        } else {
                            alert(data.message || 'No material request found.');
                            $('.loadStoreChallanDetailSection').html('');
                        }
                    },

                    error: function() {
                        alert('Failed to fetch material requests. Please try again.');
                        $('.loadStoreChallanDetailSection').html('');
                    }
                });
            }
        }

        function hideDetails() {
            document.getElementById('purchaseOrderDetails').style.display = 'none';
        }


        function processSelectedOrder() {
            const selectedOrderId = $('input[name="selected_order"]:checked').val();
            if (!selectedOrderId) {
                alert('Please select a purchase order to proceed.');
                return;
            }

            // Example action: Log selected order ID or make an AJAX request
            console.log('Selected Order ID:', selectedOrderId);

            // You can replace this with an AJAX call or other functionality
            $.ajax({
                url: '/good-receipt-notes/processPurchaseOrder',
                type: 'POST',
                data: {
                    orderId: selectedOrderId,
                    _token: '{{ csrf_token() }}' // Include CSRF token for POST requests
                },
                success: function(response) {
                    alert('Purchase order processed successfully!');
                    console.log(response);
                },
                error: function() {
                    alert('Failed to process the purchase order. Please try again.');
                }
            });
        }
    </script>
@endsection
